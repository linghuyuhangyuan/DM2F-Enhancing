...
class DM2FNet(Base):
    def __init__(self, use_refine=False, use_sep=False, num_features=128, arch='resnext101_32x8d'):
    ...
    def forward(self, x0, x0_hd=None):
        ...
        # MLF (multi-layer features)
        concat = torch.cat((down1, down2, down3, down4), 1)
        # print(concat.shape)
        
        n, c, h, w = down1.size()

        # attentional feature integration module (AFIM) 
        ...
        x_phy = ((x0 - a * (1 - t)) / t.clamp(min=1e-8)).clamp(min=0., max=1.)
        if self.use_refine:
            x_phy = self.refine_net(x_phy, x0)
        ...
        if self.use_refine:
            if self.use_sep:
                x_j1 = self.refine_net1(x_j1, x0)
            else:
                x_j1 = self.refine_net(x_j1, x0)
        ...
        if self.use_refine:
            if self.use_sep:
                x_j2 = self.refine_net2(x_j2, x0)
            else:
                x_j2 = self.refine_net(x_j2, x0)
        ...
        if self.use_refine:
            if self.use_sep:
                x_j3 = self.refine_net3(x_j3, x0)
            else:
                x_j3 = self.refine_net(x_j3, x0)
        ...
        if self.use_refine:
            if self.use_sep:
                x_j4 = self.refine_net4(x_j4, x0)
            else:
                x_j4 = self.refine_net(x_j4, x0)
        ...
        # x_fusion = self.unet(x_fusion)
        # print(x_fusion.shape)
        
        # if self.use_refine:
        #     x_fusion = self.refine_net(x_fusion, x0)
...
class DM2FNet_woPhy(Base_OHAZE):
    def __init__(self, use_refine=False, use_sep=False, use_final=False, num_features=64, arch='resnext101_32x8d'):
    ...
    def forward(self, x0):
        ...
        # x_fusion = self.unet(x_fusion)
        if self.use_final:
            x_fusion = self.refine_net_final(x_fusion, x)
