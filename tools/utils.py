...
def sliding_forward(net, x: torch.Tensor, x2=None, special=False, crop_size=1536, device="cuda:0"):
    n, c, h, w = x.size()
    ...
    if h <= crop_size and w <= crop_size:
        ...
        result = torch.zeros(n, c, h, w).to(device)
        count = torch.zeros(n, 1, h, w).to(device)
        ...
        for h_idx in range(h_steps):
            for w_idx in range(w_steps):
                ...
                if special:
                    result[:, :, hs0: hs1, ws0: ws1] += net(x[:, :, hs0: hs1, ws0: ws1], x2[:, :, hs0: hs1, ws0: ws1]).data
                else:
